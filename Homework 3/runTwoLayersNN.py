...
lRates = [8e-8, 1e-7, 5e-6, 5e-3]
regStrengths = [7e1, 6e2, 4e3, 5e-3]
numClasses = np.max(yTrain) + 1
hiddenNeurons = 100
it = 1500
for lrs in lRates:
    for rst in regStrengths:
        classifier = TwoLayersNN(xTrain.shape[1], hiddenNeurons, numClasses)
        classifier.train(xTrain, yTrain, lr=lrs, reg=rst, iterations=it, verbose=True)
        accuracyVal = classifier.calAccuracy(xVal, yVal)

        if bestAcc < accuracyVal:
            bestAcc = accuracyVal
            bestModel = classifier
            bestParameters = [lrs, rst]
