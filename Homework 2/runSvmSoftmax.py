...
learning_rates = [2e-8,3e-9, 4e-10, 1e-7, 5e-6]
regularization_strengths = [1e1,2e2,3e2,4e3, 5e4]
numClasses = np.max(yVal) + 1
iters = 1500
for lrt in learning_rates:
    for rs in regularization_strengths:
        classifier = Svm(xVal.shape[1], numClasses)
        classifier.train(xTrain, yTrain, lr=lrt, reg=rs, iter=iters, verbose=True)
        acc_val = classifier.calAccuracy(xVal, yVal)

        if bestAcc < acc_val:
            bestAcc = acc_val
            bestModel = classifier
            bestParameters = [lrt, rs]
...
for lrt in learning_rates:
    for rs in regularization_strengths:
        classifier = Softmax(xVal.shape[1], numClasses)
        classifier.train(xTrain, yTrain, lr=lrt, reg=rs, iter=iters, verbose=True)
        acc_val = classifier.calAccuracy(xVal, yVal)
